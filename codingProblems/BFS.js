...
class BinarySearchTree {
  ...
  breadthFirstSearch(){
      ...
      let list = [];
      let queue = [];
      //to keep track of lvl so we can access child 
      //nodes and backtrack
      queue.push(currentNode);//putting root node in

      while(queue.length > 0) {          
          currentNode = queue.shift();//take 1st item in queue
          console.log(currentNode.value)
          list.push(currentNode.value)//which is 9 in list
          if (currentNode.left) {
              queue.push(currentNode.left);//added 4 to queue
          }
            if(currentNode.right){
                queue.push(currentNode.right);//20
            }
      }
      return list;
  }
  breadthFirstSearchR(queue, list){
      ...
      let currentNode = queue.shift();//grab currentnode
     ...
     //this = use this method within THIS class
  }
}
...
tree.breadthFirstSearchR([tree.root], [])//<--list
