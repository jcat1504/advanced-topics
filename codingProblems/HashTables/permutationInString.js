...
//2. initialize the two pointers l and r both pointing to zero and start searching s2
//3. expand r pointer until we get a desirable window(window that contains all letters of s1)
//4. if the window's length is equal to the length of s1, return true. else, move l pointer ahead one by one until we no longer have a desirable window.
    // While doing so, keep checking if the window's updated length equals the length of l, because if so, we'll return true
